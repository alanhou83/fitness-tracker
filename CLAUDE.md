...
- **Calorie formula** — `calcBurnKcal()`: reps type uses `totalReps × (getExSecPerSet(id) / getExGoalReps(id)) / 3600 × MET × weight`; hold type uses `totalSecs / 3600 × MET × weight`; run type uses `totalKm × weight × 1.036`; final result: `Math.round(strengthTotal * getBurnFactor() + runTotal)` — run is excluded from burnFactor because `km×kg×1.036` is already a complete metabolic estimate (unlike MET which only counts pure movement time); `getExSecPerSet(id)` reads `customTargets[id].secPerSet` with fallback to `EX[id].secPerSet`
- **burnFactor — design rationale** — Pure MET formula only counts active movement time (纯动作时间), which systematically underestimates true calorie cost. Two components are missing: (1) **EPOC** (运动后过量氧耗, excess post-exercise oxygen consumption): elevated metabolism for hours after training, ~+15% extra burn; (2) **Transition time** (组间休息+动作切换): time between sets and exercises where HR stays elevated; accounts for ~+25%. Together they push the real-world multiplier to ~1.4 for user's typical mixed session (184 cm, 75 kg, ~19 exercises). Calibration: pure-MET result for a full standard session is ~179 kcal; observed/expected real-world cost ~250 kcal; ratio = 250 ÷ 179 ≈ 1.4. **Adjust lower** (1.1–1.2) for very light / short sessions or when user wants conservative estimates. **Adjust higher** (1.5–1.7) for high-intensity back-to-back circuits with minimal rest. Default 1.4 is appropriate for user's current training pattern. Health implication: accuracy matters — if displayed burn is too low, user may over-train or under-eat; if too high, they may under-train or over-eat.
- **burnFactor — implementation** — `getBurnFactor()` reads `profile.burnFactor` (default 1.4); `adjBurnFactor(delta)` adjusts ±0.1, clamps 1.0–2.0, rounds to 1 decimal, saves to profile; settings UI in 个人信息 as `<span id="burn-factor-val">` with − / + buttons; `renderSettings()` populates it
